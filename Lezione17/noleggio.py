...
Questa classe deve avere come attributi una lista di film contenuti in negozio (film_list), un dizionario (rented_film)
 che ha come chiave un numero intero rappresentante l'id del cliente che ha affittato il film e per valore una lista contenente i film affittati dal cliente.
...
- Definire i seguenti metodi:
    ...
    Se il film è disponibile in negozio stampare: "Il film scelto è disponibile: {titolo_film}!". Se il film non è disponibile in negozio stamapre: 
    "Il film scelto è disponibile: {titolo_film}!".
...
class Noleggio:
    def __init__(self, film_list: list[Film], rented_film: dict):
        self.film_list: list[Film] = film_list
        self.rented_film: dict[str:list] = rented_film
        self.film_list = []
        self.rented_film = {}
    ...
    def rentAMovie(self, film, clientID):
        if film in self.film_list:
            ...
            self.rented_list[clientID].film
            print(f"Il cliente {clientID} ha noleggiato {film.getTitle}!")
        else:
            print("Non è possibile nolegiare il film {film.getTitle}!")
    ...
    def giveBack(self, film, clientID, days):
        if film in self.rented_film[clientID]:
            ...
            tot = days * 
            print("Cliente: {clientID}! La penale da pagare per il film {titolo_film} e' di {tot} euro!")